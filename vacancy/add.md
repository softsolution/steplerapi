...
| **langs** | Знание языков. Массив значений в формате array(0 => array('id' => Id языка, 'level' => Id уровня владения)). Id языков берется из справочника [job.get_langs](job/get_langs.md), id уровней владения берется из справочника  [job.get_langs_levels](job/get_langs_levels.md) | array |
